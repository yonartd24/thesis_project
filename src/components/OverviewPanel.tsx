...
function DistributionList({ title, rows }: DistributionListProps) {
  return (
    <div className="grid gap-2">
      <h5 className="m-0 text-[12px] font-black uppercase tracking-[0.18em] text-black">{title}</h5>
      {rows.length ? (
        rows.map((row) => (
          <div key={`${title}-${row.label}`} className="flex items-center justify-between gap-3 text-[12px] text-black/70">
            <span>{row.label}</span>
            <strong className="text-black">{row.count} people</strong>
          </div>
        ))
      ) : (
        <p className="m-0 text-[12px] text-black/45">No data available.</p>
      )}
    </div>
  );
}
...
function OverviewStageCard({ panel, accent, index }: OverviewStageCardProps) {
  ...
  return (
    <article
      ref={ref}
      className="grid gap-6 rounded-[30px] bg-white px-4 py-5 shadow-[0_24px_50px_rgba(16,16,16,0.08)] ring-1 ring-black/8 transition-[opacity,transform] duration-700 md:px-6 md:py-6 lg:grid-cols-[minmax(0,1fr)_240px] lg:gap-10"
      style={{
    ...
    >
      ...
      <aside className="grid content-start gap-5 lg:pl-2">
        <DistributionList title="Age" rows={panel.ageRows} />
        <DistributionList title="Gender" rows={panel.genderRows} />
        <div className="grid gap-3">
          <h5 className="m-0 text-[12px] font-black uppercase tracking-[0.18em] text-black">Prompt focus</h5>
          <p className="m-0 text-[20px] font-black leading-[1.05] tracking-[-0.05em] text-black">
            {panel.prompt}
          </p>
          <div className="flex flex-wrap gap-2">
            {panel.questionLabels.map((questionLabel) => (
              <span
                key={`${panel.stage}-${questionLabel}`}
                className="rounded-full border border-black/10 bg-[#f4f1eb] px-3 py-1 text-[10px] font-bold uppercase tracking-[0.18em] text-black/55"
              >
                {questionLabel}
              </span>
            ))}
